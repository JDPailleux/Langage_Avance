	...
	*t = new int[x];
	...
	for(i = 0; i < x; i++)
	{
		for(j = 0; j < y; j++)
		{
			...
			else if(i == 'i' && j == 1) t[i][j] = 2; // si on lit 'i', a l'etat 1, on va a 2
			else if(i == 'o' && j == 2) t[i][j] = 3; // si on lit 'o', a l'etat 2, on va a 3
			else if(i == 'n' && j == 3) t[i][j] = 4; // si on lit 'n', a l'etat 3, on va a 4 accepteur
	
			cout << "i: " << i << " j: " << j << "t : " << t[i][j];
		}
	}
return t;
}
...
int main()
{
	Automate a(5 , 256 , 0 , 4, creaAutomate(266, 5));
	...
	cout << "nbre de 'tion' " << a.compteTion("tionabction poazpopozertion") << endl;
	return 0;
}